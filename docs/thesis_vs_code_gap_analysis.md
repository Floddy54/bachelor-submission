...
*Generated: 2026-05-01. **Refreshed 2026-05-11 (fourth pass)** — the Phase-2 re-run finished on the HPC; INT8 (×3), adaptive-attacker (×3), BERT-MLM v2, WAG merge (upstream) + WAG eval, BERT backdoor experiment, and BERT CROW all landed under `experiments/results/`. Several §3 "pending rerun" rows are now resolved or partially resolved; the headline new finding is that the per-model adaptive-attacker report flips model1 to **VULNERABLE** (synonym 1/25, partial 10/35, scatter 6/9), which was averaged out in the pre-rerun combined report. Previous audit dates: 2026-04-29, 2026-05-01, 2026-05-07, 2026-05-09, 2026-05-10 (first pass), 2026-05-10 (second pass), 2026-05-10 (third pass).*
...
### Phase-2 re-run completed 2026-05-11 (fourth pass)

The 11-job re-run described in `scripts/slurm_temp/README.md` finished
on 2026-05-11. The eleven `sbatch` submissions and what they produced:

| # | Command | Output landed at | Phase-2 step |
|---|---------|------------------|--------------|
| 1 | `sbatch scripts/slurm_temp/int8_eval.slurm model1` | `experiments/results/int8/model1_eval.csv` | Step 4 (model1) |
| 2 | `sbatch scripts/slurm_temp/int8_eval.slurm model2` | `experiments/results/int8/model2_eval.csv` | Step 4 (model2) |
| 3 | `sbatch scripts/slurm_temp/int8_eval.slurm model3` | `experiments/results/int8/model3_eval.csv` | Step 4 (model3) |
| 4 | `sbatch scripts/slurm_temp/adaptive_attacker.slurm model1` | `experiments/results/adaptive_attacker/adaptive_attacker_model1_{report.md,results.json}` | Step 8 (model1) |
| 5 | `sbatch scripts/slurm_temp/adaptive_attacker.slurm model2` | `…model2_{report.md,results.json}` | Step 8 (model2) |
| 6 | `sbatch scripts/slurm_temp/adaptive_attacker.slurm model3` | `…model3_{report.md,results.json}` | Step 8 (model3) |
| 7 | `sbatch scripts/slurm_temp/bert_mlm_defense_v2.slurm` | `experiments/results/bert_mlm_defense/results_v2.json` | Step 7 |
| 8 | `sbatch ANTI-BAD-CHALLENGE/classification-track/slurm_jobs/wag_merge.slurm` | `ANTI-BAD-CHALLENGE/classification-track/models/task1/wag_merged/` (upstream) | Step 5 prereq |
| 9 | `sbatch scripts/slurm_temp/bert_backdoor_experiment.slurm` | `experiments/results/bert/{poisoned_3,wag_merged}/`, `results.json` refreshed | Step 6 |
| 10 | `sbatch scripts/slurm_temp/bert_crow_defense.slurm` | `experiments/results/bert_crow_defense/{crow_bert_1,crow_bert_2,crow_bert_3}/`, `results.json` | (BERT CROW failure — Ch.6) |
| 11 | `sbatch scripts/slurm_temp/wag_eval.slurm` | `experiments/results/wag/wag/wag_merged_eval.csv` *(double-nested — see "Path issues" below)* | Step 5 |

**Phase-2 steps still NOT covered by this re-run** (so the §3 status
for them is unchanged):

- Step 1 (baseline ASR + CACC per model) — `experiments/results/asr/` and the `none` rows of `general/results_summary.csv` were not re-run; they still hold the 2026-04-15 timestamps. Not a problem for the manuscript: those numbers were already MATCH; just note they did not pass through this re-run window.
- Step 2 (TF-IDF gate run per model) — `experiments/results/general/{detection_summary.csv, gate_eval_model*.txt}` were not re-run either; same status.
- Step 9 (per-trigger / target-label investigation for App. B Table B.1) — not re-run.
- Step 10 (system-takeover, App. B Table B.2) — conditional on the manuscript decision, still unresolved.

**Phase-2 steps DROPPED from scope (2026-05-11):**

- Step 3 (Llama CROW per model) — **out of scope.** Only **BERT CROW** is in scope going forward (job #10, `experiments/results/bert_crow_defense/results.json`, which is the evidence Ch.6 §Architecture-dependence actually rests on). The Ch.5 Table 5.2 Llama-CROW row (5.44 / 1.36 / 4.76) is no longer treated as a rerun debt; the manuscript should drop, relabel, or re-source that row independently of this audit (e.g. by citing the deleted-but-historical `experiments/wanda_crow/crow_defense.json` average if the team has a saved copy, or removing the Llama-CROW column entirely from Table 5.2).

The `experiments/results/general/` tree was not wiped, contrary to the
"single source of truth" framing below — that wipe was deferred. Treat
the §0 reproducibility-plan wording as the *intended* protocol; the
*actual* execution on 2026-05-11 was an additive overlay (jobs #1–11
wrote into pre-existing trees without clearing the canonical
`results/general/`, `results/asr/`, `results/adaptive_attacker/old`
sibling files first).

**Key new findings from the re-run** (these are headline §3 / §10 changes, not bookkeeping):

1. **Adaptive attacker, model1, is VULNERABLE.** The per-model report `adaptive_attacker_model1_report.md` now reads "TF-IDF defense is VULNERABLE: Multiple evasion strategies achieved high flip rates" with synonym 1/25, **partial 10/35**, **scatter 6/9**. Pre-rerun, the audit (and the thesis) only saw the combined report's averages (0/25, 0/35, 1/9), which hid model1's susceptibility. Model2 and model3 remain ROBUST / PARTIALLY ROBUST. The Ch.5 §5.4 / App. B adaptive-attacker writeups now need a per-model split, not a single aggregate sentence. This downgrades the Ch.6 §"the TF-IDF gate is the most consistently effective defense" claim from "supported across all three models" to "supported on model2/model3; model1 is bypassed by partial-trigger and multi-word-scatter variants."
2. **BERT CROW confirms the architecture-dependence claim in Ch.6.** `experiments/results/bert_crow_defense/results.json` shows `asr_reduction_percent = 0.0` across `poisoned_{1,2,3}` (BERT pre-CROW ASR = 100 %, post-CROW ASR = 100 %, with clean accuracy moving 91.9 % → 92.6 %). This is the BERT-side failure mode Ch.6 §Architecture-dependence assumes; previously the only evidence in the repo was the Llama-side `crow_defense.json` (now deleted) and a verbal claim.
3. **BERT-MLM v2 is now sourced.** `experiments/results/bert_mlm_defense/results_v2.json` records strict 82.0 / lenient 98.0 detection, FP 9.8 / 15.2, comparison against TF-IDF (100 / 1.5) and MLM-v1 (14.7 / 88.9). The Ch.5 §5.7 BERT-MLM table is now MATCH at the artefact level.
4. **BERT cross-architecture experiment is freshly sourced.** `experiments/results/bert/results.json` now records `bert_clean = 92.8 % / 0 % ASR`, `bert_poisoned_avg = 91.9 % / 100 %`, `bert_wag = 92.2 % / 100 %` (three poisoned runs all 100 % ASR, including the new `poisoned_3`). Ch.5 §5.8 / Table 5.3 numbers reproduce.

**Path issues to fix before submission** (small, surgical):

- `wag_eval.slurm` writes to `experiments/results/wag/wag/wag_merged_eval.csv` (double-nested), but Ch.5 §5.8 footnote cites `results/wag/wag_merged_eval.csv` (single-level). Either fix the SLURM script's `--output_dir` to `experiments/results/wag/` or move/symlink the file up one level. Reproducibility-sensitive — examiners will rerun this.
- `experiments/results/int8/model{1,2,3}_eval.csv` are full per-sample eval logs (66 974 rows each), not the aggregated Table 5.2 INT8 row. They reproduce the *evaluation*, but the Table 5.2 ASR/CACC cells need a downstream aggregation script (e.g., a small `python scripts/summarize_eval.py` pass over each CSV that filters to the Anti-BAD `n=399` challenge subset and applies the same ASR convention as the upstream `baseline_*.py`). Same caveat applies to `wag_merged_eval.csv`. Until that aggregation step runs, Table 5.2 INT8/WAG rows remain THESIS-ONLY at the *aggregated number* level even though the raw eval CSVs now exist.
- `experiments/results/bert/poisoned_3/` and `bert_wag_merged/` contain model binaries (`model.safetensors`, `config.json`). These should not be committed — they fall under the standing "Don't change" / "Don't document binaries" rule.

The §3 / §10 tables below are not re-tabulated yet — read each row under
the §0 fourth-pass notes above when its status reads "pending rerun"
or "THESIS-ONLY at the artefact level."
...
|---|---|---|
| **CROW** (Llama track) | `ANTI-BAD-CHALLENGE/classification-track/scripts/baseline_crow.py` (upstream). **No CROW output file is checked in any more** (`experiments/wanda_crow/crow_defense.json` was deleted in the 2026-05-10 cleanup); **Llama CROW dropped from scope 2026-05-11** (the rerun owes only the BERT-CROW failure result). | The CROW row of Ch.5 Table 5.2 (5.44 / 1.36 / 4.76; 85.71 % CACC) is **out of scope** — manuscript decides whether to drop, relabel, or re-source. See §3. |
| **CROW** (BERT comparison track) | `src/training/bert_crow_defense.py`; **after the 2026-05-11 re-run** `experiments/results/bert_crow_defense/{crow_bert_1,crow_bert_2,crow_bert_3}/` and `results.json` (CROW yields 0 % ASR reduction on all three poisoned BERT runs; accuracy 91.9 → 92.6 %). | Cited implicitly in Ch.6 (CROW failing on BERT supports the architecture-dependence story); now sourced. |
...
| **WAG** (BERT cross-architecture) | `src/training/bert_backdoor_experiment.py` runs the full BERT WAG comparison; **refreshed 2026-05-11** `experiments/results/bert/results.json` (bert_clean 92.8 % / 0 % ASR; bert_poisoned_avg 91.9 % / 100 %; bert_wag 92.2 % / 100 %; poisoned_3 added). | Numbers in Ch.5 Table 5.3 (BERT 100→100; Llama 100→8.16) reproduce from the 2026-05-11 `results.json`. |
| **INT8 quantization** | `scripts/eval_on_csv.py` (the generic eval CLI accepts `--int8`); **after the 2026-05-11 re-run** `experiments/results/int8/model{1,2,3}_eval.csv` (per-sample logs, 66 974 rows each). | Raw eval logs now exist. The Table 5.2 INT8 row (34.69 / 1.36 / 6.80; 85.71 % CACC) still needs an aggregation step over these CSVs onto the Anti-BAD `n=399` challenge subset with the same ASR convention as the upstream baselines. Figure 5.4 (`bachelor_overleaf/.../10_int8_verification.png`) remains the qualitative anchor for the model1 34.69 % / 98.70 % protocol-sensitivity finding. |
...
| **BERT-MLM detection** (Ch.5 §5.7) | `src/training/bert_mlm_defense_v2.py`; **after the 2026-05-11 re-run** `experiments/results/bert_mlm_defense/results_v2.json` (strict 82.0 / 9.8, lenient 98.0 / 15.2; v1 14.7 / 88.9 and TF-IDF 100 / 1.5 comparison rows embedded). | Numbers in Ch.5 Table 5.2 / Ch.5 §5.7 are now backed by `results_v2.json`. |
...
| Table 5.2 CROW: model1 5.44%, model2 1.36%, model3 4.76% | **Llama CROW dropped from scope 2026-05-11.** Only the BERT CROW failure result is in scope going forward (`experiments/results/bert_crow_defense/results.json`, Ch.6 §Architecture-dependence). The Llama-side per-model CROW numbers have no checked-in file and the team is not re-staging them. | OUT OF SCOPE — the manuscript decision is to drop, relabel, or re-source the Llama-CROW row of Table 5.2 (not a "rerun debt" any more). |
| Table 5.2 INT8: model1 34.69%, model2 1.36%, model3 6.80% | **2026-05-11 re-run produced per-model eval CSVs** at `experiments/results/int8/model{1,2,3}_eval.csv` (66 974 rows each — full per-sample logs). These reproduce the *evaluation*, but the Table 5.2 cells need a downstream aggregation onto the Anti-BAD `n=399` challenge subset with the same ASR convention as the upstream baselines. Figure 5.4 (`10_int8_verification.png`) still illustrates the model1 34.69 % / 98.70 % protocol-sensitivity finding qualitatively. | THESIS-ONLY at the aggregated-number level (raw eval logs exist; aggregation step pending) |
| Table 5.2 WAG (merged): 8.16% × 3 models, 85.71% CACC | **2026-05-11 re-run produced** `experiments/results/wag/wag/wag_merged_eval.csv` (66 974 rows; **note double-nested path** vs the Ch.5 §5.8 footnote citation). Same aggregation caveat as INT8 — the 8.16 % cell still needs a per-protocol aggregation step over the new CSV. The Llama WAG merge under `ANTI-BAD-CHALLENGE/.../models/task1/wag_merged/` was re-staged by the upstream `wag_merge.slurm`. | THESIS-ONLY at the aggregated-number level (raw eval logs exist; aggregation step pending; path needs un-nesting) |
...
| §Adaptive Attacker: "TF-IDF gate detected all 25 synonym variants" | **Per-model reports after 2026-05-11 re-run**: `adaptive_attacker_model1_report.md` synonym **1/25**, partial **10/35**, scatter **6/9** ("VULNERABLE"); `…model2_report.md` synonym 0/25, partial 0/35, scatter 1/9 ("PARTIALLY ROBUST"); `…model3_report.md` 0/25, 0/35, 0/9 ("ROBUST"). The earlier combined report's "0/25 + 0/35 + 1/9" was a model-averaged view that hid model1's susceptibility. | MISMATCH — the thesis claim no longer holds for model1, and the per-model split needs to land in Ch.5 §5.4 / App. B. |
...
| §BERT-MLM: TF-IDF 100/1.5; v1 14.7/88.9; v2 strict 82.0/9.8; v2 lenient 98.0/15.2 | `src/training/bert_mlm_defense_v2.py` and (after the 2026-05-11 re-run) `experiments/results/bert_mlm_defense/results_v2.json`: strict **82.0 / 9.8**, lenient **98.0 / 15.2**, with the same v1 (14.7 / 88.9) and TF-IDF (100 / 1.5) comparison rows embedded. | MATCH |
| §Cross-architecture: BERT 100%→100% after WAG; Llama 100%→8.16% after WAG. Footnote: "Numbers verified against `results/bert/results.json` (BERT) and `results/wag/wag_merged_eval.csv` (Llama)." | `experiments/results/bert/results.json` exists ✓ (refreshed 2026-05-11; BERT-side numbers reproduce). **`results/wag/wag_merged_eval.csv` is double-nested after the re-run** — actual path is `experiments/results/wag/wag/wag_merged_eval.csv`. Either fix `wag_eval.slurm`'s `--output_dir` to write at the correct level, or move/symlink the file up one level. | MISMATCH (path — re-run produced the file but at the wrong nesting) |
